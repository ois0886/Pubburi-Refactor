...
이 문서는 주점부리(Pubburi) 리팩토링 방향과 현재 반영된 변경 사항을 기록한다.
...
- 탁주부터 위스키·사케까지 동등하게 다루는 종합 주류 큐레이션 톤으로 홈 carousel, 카테고리, 상품 카드, 상세, 장바구니, 프로필, 관리자 화면을 재설계했다.
...
- 상품 탐색 조건을 route query와 동기화해 공유 URL, 새로고침, 뒤로가기를 지원한다.
- 고객 catalog와 관리자 product/market page state를 분리해 관리자 조회가 고객 필터를 덮어쓰지 않는다.
- 공통 skeleton, empty/404, confirm dialog, quantity stepper와 pending-key UI 상태를 추가했다.
- 장바구니 수량 정규화, 로그인 후 원래 경로 복귀, 후기 수정, 아이디 중복 확인, 주문/회원 관리 기능을 기존 API에 연결했다.

## 2026-07 종합 주류 커머스 안정성 보강

- API client가 network, non-JSON, 401/403/404/409/5xx 응답을 사용자 친화적인 메시지로 정규화한다.
- mutation service는 생성자 주입과 transaction 경계를 사용한다.
- 주문 생성 중 상품, 상세, 스탬프, 회원 적립 중 하나라도 실패하면 전체 작업을 롤백한다.
- 없는 리소스의 수정/삭제는 성공 `false` 대신 `404`를 반환하고 관리자 자기 계정 삭제는 `409`로 차단한다.
